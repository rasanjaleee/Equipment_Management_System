...
const Equipment = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ── Hero Section ── full-bleed, matches home page width ── */}
      <div
      ...
      >
        {/* Gradient overlay — darker at bottom so search bar sits cleanly */}
        <div
        ...
        <div className="relative flex flex-col items-center justify-center text-center px-4 py-20">
          ...
          <span
            ...
            style={{ background: 'rgba(234,179,8,0.20)', color: '#FDE68A', border: '1px solid rgba(234,179,8,0.40)' }}
          >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex-1 min-w-[200px]">
              <div
                onClick={() => { setShowDeptDropdown(!showDeptDropdown); setShowLabDropdown(false); }}
              ...
              {showDeptDropdown && (
                <div className="absolute z-20 w-full mt-1 bg-white rounded-lg shadow-xl max-h-60 overflow-y-auto border border-yellow-100">
                  <div
                    onClick={() => { setSelectedDepartment(''); setShowDeptDropdown(false); }}
            ...
            <div className="relative flex-1 min-w-[200px]">
              <div
                onClick={() => { setShowLabDropdown(!showLabDropdown); setShowDeptDropdown(false); }}
                className="bg-white rounded-lg px-4 py-2.5 flex items-center justify-between cursor-pointer hover:bg-yellow-50 transition-colors shadow-sm"
              >
                <span className="text-gray-700 font-medium text-sm truncate">
              ...
              {showLabDropdown && (
                <div className="absolute z-20 w-full mt-1 bg-white rounded-lg shadow-xl max-h-60 overflow-y-auto border border-yellow-100">
                  <div
                    onClick={() => { setSelectedLaboratory(''); setShowLabDropdown(false); }}
                    className="p-3 hover:bg-yellow-50 cursor-pointer border-b text-sm text-gray-500 italic"
                  >
                    All Laboratories
                  </div>
                  {laboratoriesList.map((lab, index) => (
                    <div
                      key={index}
                      onClick={() => { setSelectedLaboratory(lab); setShowLabDropdown(false); }}
                      className="p-3 hover:bg-yellow-50 cursor-pointer border-b last:border-b-0 text-sm text-gray-700"
                    >
                      {lab}
                    </div>
            ...
            <div className="flex gap-2 flex-shrink-0">
              <button
                onClick={() => { setSelectedDepartment(''); setSelectedLaboratory(''); setSearchQuery(''); }}
              ...
              <button
                onClick={() => { setSelectedEquipmentForBorrow(null); setIsBorrowModalOpen(true); }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(searchQuery || selectedLaboratory || selectedDepartment) && (
          <div className="mb-6 p-3 bg-blue-50 border-l-4 border-blue-500 rounded flex flex-wrap items-center gap-2">
            <p className="text-sm text-gray-700">
              ...
              {selectedLaboratory && <span>Lab: <strong>{selectedLaboratory}</strong></span>}
              <span className="ml-2 text-gray-500">
                — <strong>{filteredEquipment.reduce((acc, lab) => acc + lab.equipment.reduce((sum, g) => sum + g.totalQuantity, 0), 0)}</strong> items
        ...
        {!loading && !error && filteredEquipment.map((lab, index) => {
          const { key: labKey, currentPage, totalPages } = getLabPage(lab.name, lab.equipment.length);
          const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
          const paginatedEquipment = lab.equipment.slice(startIndex, startIndex + ITEMS_PER_PAGE);
            ...
            <div key={index} className="mb-12">
              ...
              <div className="flex items-center gap-4 mb-6 bg-white rounded-xl p-4 shadow-sm border-l-4 border-yellow-500">
                <div className="flex-1">
                  ...
                  <p className="text-gray-500 text-sm mt-0.5">
                    {lab.equipment.reduce((acc, g) => acc + g.totalQuantity, 0)} items &nbsp;·&nbsp; {lab.equipment.length} types
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                {paginatedEquipment.map((group, groupIndex) => (
                  <div key={groupIndex} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    <div className="aspect-[4/3] bg-gray-100 overflow-hidden">
                      <img
                        ...
                        onError={(e) => { e.target.onerror = null; e.target.src = '/images/sample1.jpg'; }}
                      />
                    </div>
                    <div className="p-3">
                      <h3 className="font-semibold text-sm text-gray-900 mb-2 line-clamp-2 min-h-[2.5rem]">{group.name}</h3>
                      <div className="bg-gray-50 rounded-lg p-2 mb-3">
                        <p className="text-xs text-gray-600 mb-1.5 font-medium">Qty: {group.totalQuantity}</p>
                        <div className="flex flex-wrap gap-1.5 text-[10px]">
                          <span className="flex items-center gap-1 bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full">
                            <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>{group.working}
                          </span>
                          <span className="flex items-center gap-1 bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full">
                            <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span>{group.underRepair}
                          </span>
                          <span className="flex items-center gap-1 bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-500"></span>{group.broken}
                          </span>
                        </div>
                      </div>
                      <button
                        onClick={() => navigate(`/equipment/${group.displayItem.id}`)}
